...
export class BluetoothService {
  ...
  // 🆕 MODO DUAL: sourceMode (0=sensor, 1=digital) + mode (0=continuo, 1=pulso)
  public sourceMode$ = new BehaviorSubject<0 | 1>(0);  // 0=sensor ultrasónico, 1=input digital (D8/D9)
  public mode$ = new BehaviorSubject<0 | 1>(0);        // 0=continuo, 1=pulso
  ...
  public arduinoProtocolVersion$ = new BehaviorSubject<number>(3);
  ...
  private arduinoProtocolVersion = 3;
  ...
  async loadConfigFromPreferences(): Promise<void> {
    try {
      const [srcMode, mode, thr, hys, inD, outD, inT, actT, enL, enR, lpm, apps, grps] = await Promise.all([
      ...
      this.zone.run(() => {
        // 🆕 Source Mode
        if (srcMode.value !== null) {
          const sm = Number(srcMode.value);
          this.sourceMode$.next((sm === 1) ? 1 : 0);
        }
  ...
  async connect(deviceOrAddress: BluetoothDevice | string): Promise<void> {
    ...
    console.log(`✅ Conectado - Firmware v${this.arduinoProtocolVersion}`);
  }
  ...
  private adaptToProtocolVersion() {
    if (this.arduinoProtocolVersion >= 2) {
      this.stopHeartbeat();
    } else {
      this.startHeartbeat();
    }
  ...
  private dispatchFrame(type: number, seq: number, payload: Uint8Array) {
    ...
    switch (type) {
      ...
      case this.EVT_SNAPSHOT: {
        ...
        const srcMode = payload[4];             // 🆕 byte 4: sourceMode
        const m = payload[5] === 1 ? 1 : 0;     // byte 5: mode (era 4)
        const thr = this.u16LE(payload, 6);     // byte 6-7 (era 5-6)
        const hys = this.u16LE(payload, 8);     // byte 8-9 (era 7-8)
        const in0 = this.u16LE(payload, 10);    // byte 10-11 (era 9-10)
        const out0 = this.u16LE(payload, 12);   // byte 12-13 (era 11-12)
        const in1 = this.u16LE(payload, 14);    // byte 14-15 (era 13-14)
        const active1 = this.u32LE(payload, 16);// byte 16-19 (era 15-18)
      ...
      case this.EVT_STATUS: {
        ...
        const srcMode = payload[8];             // 🆕 byte 8: sourceMode
        const m = payload[9] === 1 ? 1 : 0;     // byte 9: mode (era 8)
        const thr = this.u16LE(payload, 10);    // byte 10-11 (era 9-10)
        const hys = this.u16LE(payload, 12);    // byte 12-13 (era 11-12)
        const in0 = this.u16LE(payload, 14);    // byte 14-15 (era 13-14)
        const out0 = this.u16LE(payload, 16);   // byte 16-17 (era 15-16)
        const in1 = this.u16LE(payload, 18);    // byte 18-19 (era 17-18)
        const active1 = this.u32LE(payload, 20);// byte 20-23 (era 19-22)
  ...
  private buildConfigPayload(c: ReturnType<BluetoothService['snapshotConfig']>): Uint8Array {
    // 🆕 Firmware v3.0: 16 bytes (era 15)
    const pl = new Uint8Array(16);
    let o = 0;

    pl[o++] = c.sourceMode & 0xFF;         // 🆕 byte 0: sourceMode
    pl[o++] = c.mode & 0xFF;               // byte 1: mode
  ...
  // 🆕 API para cambiar sourceMode
  setSourceMode(v: 0 | 1) {
    this.sourceMode$.next(v);
  ...
  async setSideEnabled(side: 'L' | 'R', enabled: boolean): Promise<void> {
    const previousState = side === 'L' 
      ? this.enabledLeft$.value 
      : this.enabledRight$.value;
    ...
    try {
      ...
      console.error('❌ SET_ENABLE failed, rolling back:', error);
      if (side === 'L') this.enabledLeft$.next(previousState);
      if (side === 'R') this.enabledRight$.next(previousState);
      throw error;
    }
  }
